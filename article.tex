...
\begin{center}
\begin{tikzpicture}
  \node[text width=0.8\hsize] at (0,0) (sig) {\texttt{\# v2 git bundle}};
  \node[inner sep=0mm,fit=(sig),draw,rounded corners,dashed,minimum width=0.8\hsize] (sigbox) {};
  \node[draw,rounded corners,fill=white] at (sigbox.north) (siglabel) {Git Bundle Signature};
  \node[text width=0.8\hsize,anchor=north west,yshift=-1mm] at (sig.south west) (b1) {\tt 3aa340a2e3d125ab6703e5c9bdfede2054a9c0c5 refs/heads/master};
  ...
  \node[rotate=-90,draw,rounded corners,fill=white] at (digestbox.east) {Digest};
\end{tikzpicture}
\end{center}
